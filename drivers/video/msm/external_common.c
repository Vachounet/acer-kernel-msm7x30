...
static uint16 external_common_hdmi_phy_addr;
u16 hdmi_common_get_phy_addr(void)
{
	return external_common_hdmi_phy_addr;
}
EXPORT_SYMBOL(hdmi_common_get_phy_addr);

static uint32 external_common_hdmi_ieee;
u32 hdmi_common_get_hdmi_ieee(void)
{
	return external_common_hdmi_ieee;
}
EXPORT_SYMBOL(hdmi_common_get_hdmi_ieee);
#endif
...
#ifdef CONFIG_MACH_ACER_A5
	external_common_hdmi_phy_addr = ((uint16)vsd[4] << 8) + (uint16)vsd[3];
	external_common_hdmi_ieee  = ((uint32)vsd[3] << 16) + ((uint32)vsd[2] << 8) + (uint32)vsd[1];
	DEV_DBG("EDID: VSD PhyAddr=%04x, MaxTMDS=%dMHz\n",
		((uint32)vsd[4] << 8) + (uint32)vsd[3], (uint32)vsd[6] * 5);
...
#ifdef CONFIG_MACH_ACER_A5
	case 3:
		status = hdmi_common_read_edid_block(2, edid_buf+0x80*2);
		if (status) {
			DEV_ERR("%s: ddc read block(3) failed: %d\n", __func__,
				status);
			goto error;
		}
		break;
