...
void mdp4_overlay_vg_setup(struct mdp4_overlay_pipe *pipe)
{
	...
	if (pipe->element1 == C1_B_Cb) {
		pattern = (pipe->element3 << 24) | (pipe->element1 << 16) |
			(pipe->element0 << 8) | pipe->element2;
	} else {
		pattern = (pipe->element3 << 24) | (pipe->element2 << 16) |
		...
		int ptype, int mixer, int req_share)
{
	...
	if (found) {
		...
		/* If surface flinger uses a lot of video overlay,
		   it will use the same overlay channel */
		if( pipe->pipe_ndx == 6 && mixer == 0 )
			pipe--;
#endif
...
int mdp4_overlay_unset(struct fb_info *info, int ndx)
{
	...
	mfd->panel_info.lcd.hw_vsync_mode = FALSE;
#endif
	mutex_unlock(&mfd->dma->ov_mutex);
