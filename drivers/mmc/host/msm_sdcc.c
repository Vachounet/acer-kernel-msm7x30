...
struct workqueue_struct *msmsdcc_resume_work_queue = NULL;
struct wake_lock msmsdcc_resuming_wake_lock;
atomic_t msmsdcc_resuming_count;
static int msmsdcc_pm_resume2(struct msmsdcc_host *host);
static void msmsdcc_late_resume2(struct msmsdcc_host *host);
static void msmsdcc_early_suspend2(struct msmsdcc_host *host);
#endif
...
/* resuming thread */
#if defined(CONFIG_MMC_RESUMING_THREAD)
int msmsdcc_schedule_delayed_resume_work(struct delayed_work *work,
				     unsigned long delay)
{
	atomic_inc(&msmsdcc_resuming_count);
	wake_lock(&msmsdcc_resuming_wake_lock);
	return queue_work(msmsdcc_resume_work_queue, &work->work);
}
EXPORT_SYMBOL(msmsdcc_schedule_delayed_resume_work);

void msmsdcc_resume_work(struct work_struct *work)
{
	struct msmsdcc_host *host =
		container_of(work, struct msmsdcc_host, resume_work.work);

	msmsdcc_pm_resume2(host);
	if (atomic_dec_return(&msmsdcc_resuming_count) == 0)
		wake_unlock(&msmsdcc_resuming_wake_lock);
}

void msmsdcc_check_resuming_wake_lock(void)
{
	if (atomic_dec_return(&msmsdcc_resuming_count) == 0)
		wake_unlock(&msmsdcc_resuming_wake_lock);
}
EXPORT_SYMBOL(msmsdcc_check_resuming_wake_lock);

#ifdef CONFIG_HAS_EARLYSUSPEND
void msmsdcc_late_resume_work(struct work_struct *work)
{
	struct msmsdcc_host *host =
		container_of(work, struct msmsdcc_host, late_resume_work.work);

	msmsdcc_late_resume2(host);
	if (atomic_dec_return(&msmsdcc_resuming_count) == 0)
		wake_unlock(&msmsdcc_resuming_wake_lock);
}

void msmsdcc_early_suspend_work(struct work_struct *work)
{
	struct msmsdcc_host *host =
		container_of(work, struct msmsdcc_host, early_suspend_work.work);

	msmsdcc_early_suspend2(host);
	if (atomic_dec_return(&msmsdcc_resuming_count) == 0)
		wake_unlock(&msmsdcc_resuming_wake_lock);
}
#endif
...
/* resuming thread */
#if defined(CONFIG_MMC_RESUMING_THREAD)
	if (wake_lock_active(&msmsdcc_resuming_wake_lock)) {
		flags = 0;
		msmsdcc_schedule_delayed_resume_work(&host->early_suspend_work, 0);
	}
	else {
		spin_lock_irqsave(&host->lock, flags);
		host->polling_enabled = host->mmc->caps & MMC_CAP_NEEDS_POLL;
...
#if defined(CONFIG_MMC_RESUMING_THREAD)
	flags = 0;
	msmsdcc_schedule_delayed_resume_work(&host->late_resume_work, 0);
#else
	if (host->polling_enabled) {
		spin_lock_irqsave(&host->lock, flags);
		host->mmc->caps |= MMC_CAP_NEEDS_POLL;
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	/*  The wake lock will be use in msmsdcc_platform_sdiowakeup_irq,
	  *  if we init SDIO slot first.
	  *  So the wake lock should be init first*/
...
/* resuming thread */
#if defined(CONFIG_MMC_RESUMING_THREAD)
	if (msmsdcc_resume_work_queue == NULL) {
		msmsdcc_resume_work_queue = create_singlethread_workqueue("msmsdcc_suspend_thread");
		wake_lock_init(&msmsdcc_resuming_wake_lock, WAKE_LOCK_SUSPEND, "msmsdcc_resuming_work");
		atomic_set(&msmsdcc_resuming_count, 0);
	}
	INIT_DELAYED_WORK(&host->resume_work, msmsdcc_resume_work);
	INIT_DELAYED_WORK(&host->late_resume_work, msmsdcc_late_resume_work);
	INIT_DELAYED_WORK(&host->early_suspend_work, msmsdcc_early_suspend_work);
#endif
...
/* resuming thread */
#if defined(CONFIG_MMC_RESUMING_THREAD)
	msmsdcc_schedule_delayed_resume_work(&host->resume_work, 0);
#else
	rc = msmsdcc_runtime_resume(dev);
	if (host->plat->status_irq)
		enable_irq(host->plat->status_irq);
...
#ifdef CONFIG_MACH_ACER_A5
module_param(hw_ver, int, S_IRUGO);
#endif
