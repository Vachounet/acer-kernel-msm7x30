...
static void cypress_work_func(struct work_struct *work)
{
	...
	for (i = 0; i < points; i++) {
		...
		input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (pos[i].x > ts->abs_x_max) ? 0 : pos[i].x);
		input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (pos[i].y > ts->y_max) ? 0 : pos[i].y);
		
		
		 input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, (pos[i].z > ts->abs_pressure_max) ? 0 : pos[i].z/2);
		
		
		input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 255);
		input_report_abs(ts->input_dev, BTN_TOUCH, 255);
	...
	struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, pdata->abs_id_min, pdata->abs_id_max, 0, 0);
