...
int msm_dmov_exec_cmd_polling(unsigned id, unsigned int cmdptr)
{
	...
	writel(cmdptr, DMOV_REG(DMOV_CMD_PTR(ch), adm));
	while(!(readl(DMOV_REG(DMOV_STATUS(ch), adm)) & DMOV_STATUS_RSLT_VALID)) ;

	n = readl(DMOV_REG(DMOV_STATUS(ch), adm));

	while(DMOV_STATUS_RSLT_COUNT(n)) {
		n = readl(DMOV_REG(DMOV_RSLT(ch), adm));
		if(n != 0x80000002) {
			printk(KERN_INFO "ERROR: result: %x\n", n);
			printk(KERN_INFO "ERROR:  flush: %x %x %x %x\n",
			readl(DMOV_REG(DMOV_FLUSH0(DMOV_NAND_CHAN), adm)),
			readl(DMOV_REG(DMOV_FLUSH1(DMOV_NAND_CHAN), adm)),
			readl(DMOV_REG(DMOV_FLUSH2(DMOV_NAND_CHAN), adm)),
			readl(DMOV_REG(DMOV_FLUSH3(DMOV_NAND_CHAN), adm)));
		}
		n = readl(DMOV_REG(DMOV_STATUS(ch), adm));
	}
	spin_unlock_irqrestore(&dmov_conf[adm].lock, irq_flags);
